...
Examining all sequences which align with Barrel 10, Bullet 1, Land 3 of Hamby 224 (10-2-5 from Hamby 224; 10-1-6, 10-2-6 from Hamby 36; 10-1-1, 10-2-1 from Hamby44), we see substantial correspondance between the signatures. 
```{r}
b10sub <- barrel10 %>%
...
res_align <- res_align %>% bind_rows() %>%
  mutate(Barrel = id %>% str_extract("^ ?\\d{1,}") %>% as.numeric())

res_align_joined <- res_align %>%
  unnest(align) %>%
  select(-data, -data1, -letter1) %>%
  group_by(Barrel, letter) %>%
  summarize(lands = list(full_join_all(lands)))

res_align_plotable <- res_align_joined %>%
  mutate(longalign = map(lands, gather, key = id, value = sig, -x),
...
res_align_cycle_split <- res_align_cycle %>% 
  group_by(id, .chunk) %>% 
  mutate(chunksplit = (row_number() == 1 & .chunk != 0)) %>%
  ungroup() %>%
  filter(chunksplit) %>%
...
ggplot() + 
  ...
  geom_line(aes(x = x, y = med), color = 'grey30', data = res_align_plotable_med) + 
...
ggplot() + 
  ...
  geom_line(aes(x = x, y = sig, color = factor(line_id)), data = res_align_plotable) + 
  geom_line(aes(x = x, y = med), color = 'grey30', data = res_align_plotable_med) + 
  geom_vline(aes(xintercept = xold, color = factor(line_id)), data = res_align_cycle_split) + 
  scale_color_discrete(na.value = 'grey', guide = F) + 
  theme(axis.title = element_blank()) + 
  ggtitle("Hamby Barrel 10 Bullets")
```
